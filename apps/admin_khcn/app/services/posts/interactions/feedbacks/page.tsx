...
export default function FeedbacksPage() {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      ...
      case 'READ':
        return <Badge className="bg-blue-100 text-blue-800 border-blue-200">Đã xem</Badge>;
      default:
        return <Badge className="bg-orange-100 text-orange-800 border-orange-200 animate-pulse">Mới</Badge>;
    }
  };
  ...
  const getTypeBadge = (type: string) => {
    switch (type) {
      case 'DRAFT_DOC':
        return <Badge variant="outline" className="text-purple-600 border-purple-100 bg-purple-50">Dự thảo văn bản</Badge>;
      case 'SERVICE':
        return <Badge variant="outline" className="text-blue-600 border-blue-100 bg-blue-50">Dịch vụ công</Badge>;
      default:
        return <Badge variant="outline" className="text-gray-600 border-gray-100 bg-gray-50">Góp ý chung</Badge>;
    }
  };
        ...
        <table className="min-w-full divide-y divide-gray-200">
          <thead className="bg-gray-50/50">
            <tr>
              ...
              <th className="px-6 py-4 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider text-right">Thao tác</th>
            </tr>
          </thead>
              ...
              <tr key={f.id} className={`hover:bg-gray-50/50 transition-colors ${f.status === 'NEW' ? 'bg-orange-50/20' : ''}`}>
                <td className="px-6 py-4">
                  <div className="flex flex-col">
                    ...
                    <span className="text-xs text-gray-500 flex items-center mt-1">
                        <Mail className="w-3 h-3 mr-1" /> {f.senderEmail}
                    </span>
                  </div>
                </td>
                <td className="px-6 py-4">
                ...
                <td className="px-6 py-4 whitespace-nowrap text-right">
                  <div className="flex justify-end space-x-2">
                    <Button variant="outline" size="sm" onClick={() => openDetailDialog(f)}>
          ...
          {selectedFeedback && (
            <div className="space-y-6 py-4">
                ...
                <div>
                    ...
                    <div className="flex items-center space-x-3 mt-2">
                        ...
                        <span className="text-xs text-gray-400 flex items-center">
                            <Calendar className="w-3 h-3 mr-1" /> {new Date(selectedFeedback.createdAt).toLocaleString('vi-VN')}
                        </span>
                    </div>
                </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-gray-50 p-3 rounded-lg border border-gray-100">
                    ...
                    <div className="flex items-center text-sm font-semibold text-gray-700">
                        <User className="w-4 h-4 mr-2 text-gray-400" /> {selectedFeedback.senderName}
                    </div>
                </div>
                <div className="bg-gray-50 p-3 rounded-lg border border-gray-100">
                    <p className="text-[10px] text-gray-400 uppercase font-bold mb-1">Email liên hệ</p>
                    <div className="flex items-center text-sm font-semibold text-gray-700">
                        <Mail className="w-4 h-4 mr-2 text-gray-400" /> {selectedFeedback.senderEmail}
                    </div>
                </div>
              </div>
              ...
              <div className="space-y-2">
                 ...
                 <div className="p-4 bg-gray-50 rounded-xl border border-gray-100 text-gray-800 leading-relaxed min-h-[120px]">

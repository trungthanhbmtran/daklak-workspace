...
export default function QuestionsPage() {
  ...
  const handleAnswer = async () => {
    ...
    try {
      await postsApi.answerQuestion(selectedQuestion.id, {
        answerContent,
        isPublic,
      });
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case 'ANSWERED':
        return <Badge className="bg-green-100 text-green-800 border-green-200">Đã trả lời</Badge>;
      case 'PROCESSING':
      ...
      default:
        return <Badge className="bg-yellow-100 text-yellow-800 border-yellow-200">Chờ tiếp nhận</Badge>;
    }
  };
            ...
            <div className="space-y-6 py-4">
              ...
              <div className="bg-gray-50 p-4 rounded-lg grid grid-cols-2 gap-4 text-sm">
                <div className="flex items-center text-gray-600">
                  ...
                  <span className="font-semibold mr-2">Người gửi:</span> {selectedQuestion.askedByName}
                </div>
                <div className="flex items-center text-gray-600">
                  <Phone className="w-4 h-4 mr-2" />
                  <span className="font-semibold mr-2">Số điện thoại:</span> {selectedQuestion.askedByPhone || 'N/A'}
                </div>
                <div className="flex items-center text-gray-600">
                  <Mail className="w-4 h-4 mr-2" />
                  <span className="font-semibold mr-2">Email:</span> {selectedQuestion.askedByEmail || 'N/A'}
                </div>
                <div className="flex items-center text-gray-600">
                  <MapPin className="w-4 h-4 mr-2" />
                  <span className="font-semibold mr-2">Địa chỉ:</span> {selectedQuestion.address || 'N/A'}
                </div>
              </div>
              ...
              {/* Nội dung trả lời */}
              <div>
                <Label htmlFor="answer" className="text-sm font-bold text-gray-900 mb-2 block">

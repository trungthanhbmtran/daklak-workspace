...
// ─── TaskRow — memoized để tránh re-render toàn table ────────────────────────
...
const TaskRow = memo(function TaskRow({ task, context, onSelectTask, onSmartAssign, onHoverTask }: TaskRowProps) {
  ...
  const handleSelect = useCallback(() => onSelectTask(task), [task, onSelectTask]);
  const handleHover = useCallback(() => onHoverTask?.(task), [task, onHoverTask]);
  ...
  return (
    ...
    >
      {/* Title */}
      <TableCell className="px-6 py-5 font-semibold text-slate-900 dark:text-white max-w-xs group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors">
            ...
            <div className="flex items-center">
              <Avatar className="w-7 h-7 mr-2.5 shrink-0">
                <AvatarFallback className="bg-indigo-100 text-indigo-700 text-xs font-bold">
                  {initial(task.assignerName || task.creatorName || task.creatorEmployeeCode)}
                </AvatarFallback>
              </Avatar>
          ...
          <div className="flex flex-col gap-1.5">
            <div className="flex items-center gap-2">
              <Avatar className="w-6 h-6 shrink-0">
                <AvatarFallback className="bg-indigo-100 text-indigo-700 text-xs font-bold">
                  {task.assigneeCode === 'UNASSIGNED' ? '?' : initial(task.assigneeName || task.assigneeCode)}
                </AvatarFallback>
              </Avatar>
              <span className="font-medium text-sm truncate">
            ...
            {task.progress !== undefined && (
              <div className="flex items-center gap-2">
                <div className="flex-1 bg-slate-100 rounded-full h-1.5 overflow-hidden">
                  <div
                    className={`h-1.5 rounded-full ${task.progress === 100 ? 'bg-emerald-500' : 'bg-indigo-500'}`}
              ...
              <div className="flex items-center">
                <Avatar className="w-7 h-7 mr-2.5 shrink-0">
                  <AvatarFallback className="bg-amber-100 text-amber-600 text-xs font-bold">
                    {initial(task.supervisorName || task.supervisorCode)}
                  </AvatarFallback>
                </Avatar>
              ...
              <div className="flex items-start mt-1.5">
                <Avatar className="w-7 h-7 mr-2.5 shrink-0">
                  <AvatarFallback className="bg-teal-100 text-teal-600 text-xs font-bold">
                    {initial(task.approverName || task.approverCode)}
                  </AvatarFallback>
                </Avatar>
      ...
      <TableCell className="px-6 py-5">
        <div className={`inline-flex items-center gap-2 px-3 py-1.5 rounded-xl border ${dueInfo.bg} ${dueInfo.border} ${dueInfo.color}`}>
          ...
          <div className="flex flex-col leading-tight">
            {dueInfo.text && (
              <span className="text-[10px] font-bold uppercase tracking-wider opacity-90 mb-0.5">{dueInfo.text}</span>
            )}
      ...
      <TableCell className="px-6 py-5 text-right">
        <div className="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
          {context === 'PENDING_ASSIGN' && (!task.assigneeCode || task.assigneeCode === 'UNASSIGNED') && (
            <Button size="sm" className="h-8 rounded-lg text-xs bg-amber-500 hover:bg-amber-600 text-white font-medium" onClick={handleAssign}>
              <UserCheck className="h-3.5 w-3.5 mr-1.5" /> Phân công
            </Button>
          )}
          {context === 'MY_EXECUTION' && task.status !== 'DONE' && (
            <Button size="sm" className="h-8 rounded-lg text-xs bg-indigo-500 hover:bg-indigo-600 text-white font-medium" onClick={handleSelect}>
              <CheckCircle2 className="h-3.5 w-3.5 mr-1.5" /> Thực hiện
            </Button>
          )}
          {context === 'I_ASSIGNED' && (
            <Button size="sm" variant="outline" className="h-8 rounded-lg text-xs font-medium" onClick={handleSelect}>
              <BarChart3 className="h-3.5 w-3.5 mr-1.5" /> Tiến độ
            </Button>
          )}
          <Button variant="outline" size="icon" onClick={handleSelect} className="h-8 w-8 rounded-lg">
  ...
  return (
    <Card className="border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 rounded-2xl overflow-hidden mt-6 shadow-sm">
      ...
      <ScrollArea className="w-full">
        <Table>
          <TableHeader>
            <TableRow className="text-xs text-slate-500 uppercase bg-slate-50/50 dark:bg-slate-800/50 backdrop-blur-md">
              <TableHead className="px-6 py-5 font-bold tracking-wider">Tên công việc</TableHead>
              <TableHead className="px-6 py-5 font-bold tracking-wider">Trạng thái</TableHead>
              <TableHead className="px-6 py-5 font-bold tracking-wider">Mức ưu tiên</TableHead>
              <TableHead className="px-6 py-5 font-bold tracking-wider">
                {context === 'PENDING_ASSIGN' ? 'Người tạo / Kế hoạch' :
                 context === 'I_ASSIGNED' ? 'Người thực hiện / Tiến độ' :
                 'Thực hiện / Chỉ đạo'}
              </TableHead>
              <TableHead className="px-6 py-5 font-bold tracking-wider">Hạn chót</TableHead>
              <TableHead className="px-6 py-5 text-right font-bold tracking-wider">Thao tác</TableHead>
            </TableRow>
          </TableHeader>
          <TableBody>
            {tasks.map((task: any) => (

...
export function TaskCreateClient() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!assignee) return toast.error('Vui lòng chọn người thực hiện!');
      if (isOverload) return toast.error('Cảnh báo: Khối lượng công việc vượt quá định mức!');
      await createTask({ assigneeCode: assignee, title: taskName, taskName, weight: taskWeight, startDate, dueDate, priority, baseScore, templateId: selectedTemplateId, planId: finalPlanId });
    ...
    <div className="max-w-6xl mx-auto p-4 md:p-6 space-y-6">
      <div className="flex items-center gap-4">
        ...
        <h1 className="text-2xl font-black text-slate-900 flex items-center gap-2">
          <PlusCircle className="text-slate-600" /> Giao Việc Mới
        </h1>
      </div>
        ...
        <div className="lg:col-span-8 space-y-6">
          <Card className="border-slate-200 shadow-sm">
            <div className="bg-slate-50 px-4 py-3 border-b border-slate-100 flex items-center gap-2">
              ...
              <h3 className="font-bold text-slate-800">Thông tin công việc</h3>
            </div>
            <CardContent className="p-6">
              <form id="taskForm" onSubmit={handleSubmit} className="space-y-6">
                    ...
                    <div className="space-y-2 p-4 bg-slate-50 border border-slate-200 rounded-xl">
                      ...
                      <div className="mt-3 grid gap-3 max-h-[300px] overflow-y-auto pr-1">
                        {uniqueTasks.map((t: any) => (
                          <div
                            key={t.id || t.title}
                            className="p-4 rounded-xl border border-slate-200 bg-white flex flex-col md:flex-row gap-4 items-start md:items-center justify-between"
                          >
                            <div className="flex-1">
                              <div className="font-bold text-sm text-slate-800">{t.title}</div>
                              <div className="text-xs text-slate-500 mt-1 flex flex-wrap gap-2">
                                <span>Ưu tiên: <span className={`font-medium ${t.priority === 'HIGH' ? 'text-red-600' : t.priority === 'LOW' ? 'text-green-600' : 'text-slate-600'}`}>{t.priority === 'HIGH' ? 'Cao' : t.priority === 'LOW' ? 'Thấp' : 'Trung bình'}</span></span>
                                <span>•</span>
                ...
                {(!planId || planId === 'none') && (
                  <>
                    <div className="space-y-2">
                      ...
                      <Input value={taskName} onChange={e => setTaskName(e.target.value)} required className="h-11 bg-white" placeholder="Nhập tên công việc cần giao..." />
                    </div>
                ...
                {(!planId || planId === 'none') && (
                  <Button type="submit" form="taskForm" className="w-full mt-4 bg-slate-900">
                    Giao việc ngay
                  </Button>
                )}
              </form>
          ...
          <div className="lg:col-span-4 space-y-6">
            <Card className="sticky top-6 border-slate-200">
              <CardContent className="p-4 space-y-4">
                <div className="space-y-2">
                  <div className="flex items-center justify-between">
                    ...
                    <Button
                      variant="outline"
                      size="sm"
                      className="h-6 text-[10px] px-2 bg-white text-slate-700"
                      onClick={() => {
                      ...
                      type="button"
                    >
                      <Sparkles className="w-3 h-3 mr-1" /> Giao nhanh
                    </Button>
                  </div>
                  <Popover open={openAssignee} onOpenChange={setOpenAssignee}>
                    <PopoverTrigger asChild>
                      <Button
                        ...
                        className="w-full justify-between h-11 bg-white border-slate-200"
                      >
                ...
                {selectedEmp && (
                  <div className={`space-y-4 p-4 rounded-xl border border-slate-200`}>
                    {/* Thông tin nhân sự */}
                    <div className="flex items-start gap-3">
                      <div className={`w-10 h-10 rounded-full flex items-center justify-center font-bold text-lg shrink-0 bg-slate-100 text-slate-600`}>
                    ...
                    <div className="space-y-2 pt-2 border-t border-slate-200">
                      <div className="flex justify-between items-center text-xs font-bold text-slate-600">
                        ...
                        <span>{newLoad} / {selectedEmp.rankLimit} đ</span>
                      </div>

...
export function JobTitleConfigDialog({ open, onOpenChange, jobTitle, domainId, onDomainIdChange, onSave, isSaving, domainsForUnit, unitName }: Props) {
  return (
      ...
      <DialogContent className="max-w-[500px] max-h-[90vh] flex flex-col gap-0 p-0 overflow-hidden shadow-lg border-primary/20">
        <DialogHeader className="px-6 pt-6 pb-4 shrink-0 bg-muted/10 border-b">
          <DialogTitle className="text-lg flex items-center gap-2 text-primary">
            ...
            Tieu chuan chuc danh {jobTitle ? `— ${jobTitle.name}` : ""}
          </DialogTitle>
          <DialogDescription className="mt-2">Thiet lap pham vi quan ly mac dinh cho chuc danh nay.</DialogDescription>
        </DialogHeader>
          ...
          <div className="space-y-6 px-6 py-5 overflow-y-auto flex-1 min-h-0 bg-background">
            <div className="flex gap-3 items-start bg-blue-50 text-blue-800 p-3.5 rounded-lg border border-blue-100 text-sm">
              ...
              <p className="leading-relaxed">
                Day la cau hinh <strong>khung tieu chuan chung</strong> cho toan bo cac vi tri thuoc chuc danh <em>{jobTitle.name}</em>. Ban co the tinh chinh lai chi tiet nhiem vu cho tung ca nhan cu the o phan <strong>&quot;Phan cong nhiem vu chi tiet&quot;</strong>.
              </p>
            </div>
            <div className="space-y-3">
              <label className="text-sm font-semibold flex items-center gap-2 text-foreground">
                <Network className="h-4 w-4 text-muted-foreground" />
                Linh vuc phu trach chung
              </label>
              <Select value={domainId} onValueChange={onDomainIdChange}>
                <SelectTrigger className="h-10 bg-background"><SelectValue placeholder="-- Chon linh vuc phu trach --" /></SelectTrigger>
                <SelectContent>
                  <SelectItem value="__none__">Khong quy dinh linh vuc</SelectItem>
                  {domainsForUnit.map(d => <SelectItem key={d.id} value={String(d.id)}>{d.name}</SelectItem>)}
                </SelectContent>
              </Select>
              {domainsForUnit.length === 0 && unitName != null && (
                <p className="text-xs text-amber-600 bg-amber-50 p-2 rounded border border-amber-100 mt-2">Don vi chua duoc giao linh vuc. Vui long cap nhat thong tin don vi truoc.</p>
              )}
            </div>
          </div>
        )}

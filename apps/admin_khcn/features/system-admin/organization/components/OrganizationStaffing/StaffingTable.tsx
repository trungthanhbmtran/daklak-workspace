      ...
      <Table>
        <TableHeader>
          <TableRow className="bg-muted/50 hover:bg-muted/50">
            <TableHead className="font-semibold text-foreground w-[260px]">Chuc danh</TableHead>
            <TableHead className="font-semibold text-foreground">Linh vuc phu trach</TableHead>
            <TableHead className="text-right font-semibold text-foreground w-24">Dinh bien</TableHead>
            <TableHead className="text-right font-semibold text-foreground w-20">Hien co</TableHead>
          </TableRow>
        </TableHeader>
        <TableBody>
          {report.map(row => {
                          ...
                          <div className="p-4 flex flex-col md:flex-row gap-5">
                            <div className="w-full md:w-[200px] shrink-0 space-y-2">
                              <div className="text-xs font-bold uppercase tracking-wider text-muted-foreground flex items-center gap-1 px-2 py-1">
                                <UserCheck className="h-3.5 w-3.5" /><span>Chon vi tri ({row.quantity})</span>
                              </div>
                                  ...
                                  return (
                                    <Button key={slotOrder} type="button" onClick={() => setActiveSlotMap(p => ({ ...p, [row.id]: slotOrder }))}
                                      ...
                                      <div className="flex items-center justify-between w-full">
                                        <span className="font-semibold">Vi tri {slotOrder}</span>
                                      ...
                                      <div className={cn("mt-0.5 truncate max-w-full text-[11px]", isSel ? "text-primary-foreground/90" : "text-muted-foreground", existing?.assignedEmployeeName ? "" : "italic opacity-70")}>
                                        {existing?.assignedEmployeeName || "Chua co nhan su"}
                                      </div>
                                    </Button>
                                  );
                                })}

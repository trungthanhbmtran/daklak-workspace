...
export function OrganizationStaffing() {
  ...
  const detailQuery = useQuery({
    queryKey: code
      ? [...organizationQueryKeys.all, "detail", code]
      : [...organizationQueryKeys.all, "detail"],
  ...
  const [configJobTitle, setConfigJobTitle] = useState<JobTitleItem | null>(
    null,
  );
    ...
    updateJobTitle.mutate(
      {
        ...
        domainId:
          configDomainId === "__none__" ? 0 : parseInt(configDomainId, 10),
      },
      {
        onSuccess: () => {
          setConfigOpen(false);
  ...
  const isDetailLoading =
    (detailQuery.isPending || detailQuery.isFetching) && !unit;
  const isDetailError =
    detailQuery.isError || (!!code && !isDetailLoading && !unit);
      ...
      <div className="flex flex-col flex-1 min-h-0 gap-4 mt-2">
        <section className="shrink-0 rounded-lg border bg-muted/30 p-4">
          <h3 className="text-sm font-medium mb-3">
            Thêm định biên {label.toLowerCase()}
          </h3>
          <form
            ...
            <>
              ...
              {totalPages > 1 && (
                <div className="shrink-0 flex items-center justify-between mt-2 px-1">
                  <span className="text-xs text-muted-foreground">
                    {(safeP - 1) * PAGE_SIZE + 1}–
                    {Math.min(safeP * PAGE_SIZE, reportList.length)} /{" "}
                    {reportList.length}
                  </span>
        ...
        {titleList.length > 0 && (
          <section className="shrink-0 rounded-lg border bg-muted/20 p-4">
            ...
            <p className="text-xs text-muted-foreground mb-3">
              Lĩnh vực phụ trách (theo cấp trên giao). Theo dõi phòng ban: đơn
              vị trực thuộc.
            </p>
      ...
      {isDetailLoading ? (
        ...
        <div className="rounded-xl border border-dashed bg-muted/20 py-16 flex flex-col items-center gap-3 text-center">
          ...
          <p className="text-sm text-muted-foreground">
            Không tải được thông tin đơn vị. Vui lòng thử lại.
          </p>
        </div>
      ) : isError ? (

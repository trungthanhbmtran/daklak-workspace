  ...
  return (
    <Card className="border border-border shadow-xl bg-card rounded-2xl overflow-hidden">
      <CardHeader className="border-b border-border bg-muted/30 p-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          <CardTitle className="text-lg sm:text-xl font-bold text-foreground flex items-center gap-2">
            ...
            Cấu hình Mô hình AI Cá nhân
          </CardTitle>
          <p className="text-xs sm:text-sm text-muted-foreground mt-1">Hệ thống sẽ ưu tiên dùng các khóa API cá nhân của bạn. Nếu lỗi hoặc không có, hệ thống sẽ dùng cấu hình chung.</p>
        </div>
      ...
      <CardContent className="p-6 bg-muted/10">
        ...
        <Button onClick={handleAddAiProvider} variant="outline" className="w-full mt-4 h-12 border-dashed border-2 border-input text-muted-foreground font-bold hover:bg-muted hover:border-primary/50 hover:text-primary rounded-xl transition-colors" iconStart={<Plus className="w-5 h-5" />}>Thêm Cấu hình AI Dự phòng</Button>

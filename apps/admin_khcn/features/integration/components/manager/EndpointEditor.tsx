                ...
                <div className="border border-slate-200 dark:border-slate-800 rounded-xl overflow-hidden flex-1 flex flex-col">
                  <div className="flex-1 overflow-y-auto custom-scrollbar">
                    <table className="w-full text-sm text-left">
                      <thead className="bg-slate-50 dark:bg-slate-900 text-slate-500 font-semibold border-b border-slate-200 dark:border-slate-800 sticky top-0">
                        <tr>
                          <th className="px-4 py-2 border-r border-slate-200 dark:border-slate-800 w-[40%]">Key</th>
                          <th className="px-4 py-2 w-[50%]">Value</th>
                          <th className="px-4 py-2 w-[10%] text-center"></th>
                        </tr>
                      </thead>
                      <tbody className="divide-y divide-slate-200 dark:divide-slate-800 bg-white dark:bg-slate-950">
                        {(!selectedEndpoint.headers || selectedEndpoint.headers.length === 0) ? (
                          <tr><td colSpan={3} className="px-4 py-8 text-center text-slate-400">Không có Headers</td></tr>
                        ) : (
                    ...
                    <table className="w-full text-sm text-left">
                      ...
                      <tbody className="divide-y divide-slate-200 dark:divide-slate-800 bg-white dark:bg-slate-950">
                        {(!selectedEndpoint.params || selectedEndpoint.params.length === 0) ? (
                          <tr><td colSpan={3} className="px-4 py-8 text-center text-slate-400">Không có Query Params</td></tr>
                        ) : (

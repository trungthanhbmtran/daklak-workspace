...
export function ThemePreview() {
  ...
  const getDeviceViewportClass = () => {
    switch (previewDevice) {
      case "mobile":
        return "max-w-[375px] h-[90%] border-[6px] border-slate-800 dark:border-slate-700 rounded-[2.5rem] shadow-2xl my-4";
      case "tablet":
        return "max-w-[768px] h-[95%] border-[8px] border-slate-800 dark:border-slate-700 rounded-[1.5rem] shadow-2xl my-2";
      case "desktop":
      default:
  ...
  return (
    ...
    <div className="w-full h-full bg-slate-100 dark:bg-slate-950 flex items-center justify-center p-2 transition-all duration-300 overflow-hidden">
      ...
      >
        ...
        <header className="border-b border-slate-200 dark:border-slate-800 bg-white/80 dark:bg-slate-950/80 backdrop-blur sticky top-0 z-10 px-4 py-3 flex items-center justify-between">
        ...
        <div className={`p-4 space-y-4 max-w-4xl mx-auto transition-all duration-200 ${layout.isCompact ? 'space-y-3 !p-3' : ''}`}>
          ...
          <div className="flex flex-wrap items-center gap-2 pt-1">
            ...
            <input
              ...
              className={`px-2.5 py-1 text-[11px] bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-800 shadow-inner focus:outline-none focus:ring-1 ${colors.ring} transition-all duration-200 ${getRadiusClass()} max-w-[120px]`}
            />
          </div>

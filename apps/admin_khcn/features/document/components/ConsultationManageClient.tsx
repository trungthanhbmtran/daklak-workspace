...
export function ConsultationManageClient() {
  ...
  return (
    <div className="p-6 lg:p-8 space-y-8 bg-muted/5 min-h-screen">
        ...
        <CardContent className="p-0 overflow-x-auto">
          {isLoading ? (
            <div className="p-32 flex flex-col items-center justify-center space-y-4">
              ...
              <p className="text-lg font-black text-muted-foreground animate-pulse">ĐANG TẢI DỮ LIỆU HỆ THỐNG...</p>
            </div>
            ...
            <div className="p-32 text-center flex flex-col items-center justify-center space-y-6">
              ...
              <div className="space-y-2">
                ...
                <p className="text-muted-foreground font-medium max-w-sm mx-auto">Không tìm thấy đợt lấy ý kiến nào phù hợp với bộ lọc hiện tại.</p>
              </div>
              <Button onClick={resetFilters} variant="outline" className="rounded-2xl h-12 px-8 border-primary/20 text-primary font-bold hover:bg-primary/5">Xóa toàn bộ bộ lọc</Button>
            </div>
          ) : (
            <table className="w-full text-sm text-left">
              <thead className="text-[11px] text-muted-foreground uppercase bg-muted/20 border-b border-muted/50">
                <tr>
                  <th className="px-8 py-5 font-black tracking-widest w-[40%]">Nội dung dự thảo văn bản</th>
                  <th className="px-6 py-5 font-black tracking-widest w-48">Thời hạn xử lý</th>
                  <th className="px-6 py-5 font-black tracking-widest w-72">Tiến độ phản hồi</th>
                  <th className="px-6 py-5 font-black tracking-widest text-center w-40">Tình trạng</th>
                  <th className="px-8 py-5 font-black tracking-widest text-right">Thao tác</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-muted/30 bg-background/40">
                  ...
                  return (
                    <tr key={item.id} className="hover:bg-muted/10 transition-all group">
                      <td className="px-8 py-6">
                        <p className="font-black text-foreground text-base leading-snug group-hover:text-primary transition-colors cursor-pointer line-clamp-2">{item.title}</p>
                        <div className="flex items-center gap-3 mt-1">
                          <Badge variant="outline" className="font-mono text-[9px] h-5 rounded-md border-muted-foreground/20">ID: {item.id.split('-')[0].toUpperCase()}</Badge>
                          <span className="text-[10px] font-bold text-muted-foreground flex items-center gap-1.5">
                            <Calendar className="h-3.5 w-3.5 text-primary" /> PHÁT HÀNH: {new Date(item.createdAt).toLocaleDateString('vi-VN')}
                          </span>
                        </div>
                      </td>
                      <td className="px-6 py-6">
                        <span className={cn("font-mono font-black text-sm px-3 py-1.5 rounded-xl w-fit border", isUrgent ? "bg-rose-50 text-rose-600 border-rose-200" : "bg-muted/50 text-foreground border-transparent")}>
                          {new Date(item.deadline).toLocaleDateString('vi-VN')}
                        </span>
                        {isUrgent && <div className="flex items-center gap-1 text-[10px] font-black text-rose-600 animate-pulse uppercase tracking-tight mt-1"><AlertCircle className="h-3 w-3" /> Hỏa tốc ưu tiên</div>}
                      </td>
                      <td className="px-6 py-6">
                        <div className="space-y-2.5">
                          <div className="flex items-center justify-between">
                            <span className="font-bold text-[11px] text-muted-foreground flex items-center gap-1.5 uppercase tracking-tighter">
                              <Building2 className="h-3.5 w-3.5 text-primary" /> {item.totalResponses}/{item.totalUnits} ĐƠN VỊ ĐÃ PHẢN HỒI
                            </span>
                            <span className={cn("font-black text-sm", progressPercent === 100 ? "text-emerald-600" : "text-primary")}>{progressPercent}%</span>
                          </div>
                          <div className="h-2.5 w-full bg-muted rounded-full overflow-hidden shadow-inner border border-muted/50">
                            <div className={cn("h-full transition-all duration-1000 ease-out shadow-lg", progressPercent === 100 ? "bg-emerald-500 shadow-emerald-500/20" : "bg-primary shadow-primary/20")} style={{ width: `${progressPercent}%` }} />
                          </div>
                        </div>
                      </td>
                      ...
                      <td className="px-8 py-6 text-right">
                        <div className="flex items-center justify-end gap-2">
                            ...
                            <Tooltip>
                              <TooltipTrigger asChild>
                                <Link href={`/services/documents/consultations/${item.id}`}>
                                  <Button variant="ghost" size="icon" className="h-11 w-11 rounded-2xl text-muted-foreground hover:text-primary hover:bg-primary/10 transition-all border border-transparent hover:border-primary/20">
                          ...
                          <Link href={`/services/documents/consultations/${item.id}`}>
                            <Button size="sm" className="h-11 px-5 rounded-2xl font-black text-[11px] bg-background text-foreground border-2 border-muted-foreground/10 hover:bg-muted/10 transition-all shadow-sm">
      ...
      <ConsultationCreateModal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} />
    </div>
  );
}

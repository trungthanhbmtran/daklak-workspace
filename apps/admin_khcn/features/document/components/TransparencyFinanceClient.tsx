    ...
    <div className="p-6 space-y-6 bg-muted/5 min-h-screen">
          ...
          <Card className="border shadow-sm bg-background">
            <CardContent className="p-4 flex items-center gap-4">
              ...
              <div>
                ...
                <h3 className="text-2xl font-black text-emerald-600 mt-0.5">{reports.filter((r: any) => r.status === 'PUBLISHED').length}</h3>
              </div>
            </CardContent>
          </Card>
          <Card className="border shadow-sm bg-background">
            <CardContent className="p-4 flex items-center gap-4">
              ...
              <div>
                ...
                <h3 className="text-2xl font-black text-amber-600 mt-0.5">{reports.filter((r: any) => r.status !== 'PUBLISHED').length}</h3>
              </div>
            </CardContent>
          </Card>
        </div>
      ...
      <Card className="border shadow-sm">
        <div className="p-4 border-b bg-background flex flex-wrap gap-3 items-center rounded-t-xl">
          ...
          <Button variant="outline" className="h-10 text-muted-foreground"><Filter className="h-4 w-4 mr-2" /> Lọc thêm</Button>
        </div>
          ...
          <table className="w-full text-sm text-left">
            <thead className="text-xs text-muted-foreground uppercase bg-muted/40 border-b">
              <tr>
                ...
                <th className="px-5 py-4 font-semibold w-48">Tài liệu đính kèm</th>
              ...
              {isLoading ? (
                ...
                <tr key={report.id} className="hover:bg-muted/20 transition-colors group">
                  <td className="px-5 py-4">
                    <p className="font-bold text-foreground leading-snug group-hover:text-primary transition-colors cursor-pointer mb-1">{report.abstract}</p>
                    <span className="text-[11px] font-mono font-semibold text-muted-foreground bg-muted px-1.5 py-0.5 rounded">Số: {report.documentNumber}</span>
                  </td>
                  <td className="px-5 py-4">
                  ...
                  <td className="px-5 py-4 text-center"><span className="font-mono font-bold text-foreground text-sm">{report.fiscalYear || '---'}</span></td>
                        ...
                        <Tooltip>
                          <TooltipTrigger asChild>
                            <a href={`/api/v1/media/download/${report.fileId}`} target="_blank" className="flex items-center gap-2 cursor-pointer group/file w-fit">
                              ...
                              <span className="text-xs font-medium text-foreground group-hover/file:text-primary line-clamp-1 max-w-[130px]">Tệp đính kèm.pdf</span>
                            </a>
                          </TooltipTrigger>
                  ...
                  <td className="px-5 py-4 text-center">
                    {report.status === "PUBLISHED" ? (
                      <Badge className="bg-emerald-100 hover:bg-emerald-200 text-emerald-700 shadow-none border-emerald-200"><Globe2 className="h-3 w-3 mr-1" /> Công khai</Badge>
                    ) : (
                      <Badge variant="outline" className="bg-amber-50 text-amber-700 border-amber-300 shadow-none"><Clock className="h-3 w-3 mr-1" /> Chờ duyệt</Badge>
                    )}
                  </td>
                </tr>
              ))}

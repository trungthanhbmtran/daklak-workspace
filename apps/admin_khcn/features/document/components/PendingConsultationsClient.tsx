...
export function PendingConsultationsClient() {
  return (
          ...
          <div className="divide-y divide-border/50 bg-background">
            {mockPendingConsultations.map((con) => (
              <div key={con.id} className={`p-5 hover:bg-muted/30 transition-all cursor-pointer flex items-center gap-5 ${!con.isRead ? 'bg-primary/[0.02]' : ''}`}>
                <div className="h-12 w-12 rounded-2xl bg-muted flex items-center justify-center shrink-0 border border-muted-foreground/10">
                ...
                <div className="flex-1 space-y-1.5">
                  ...
                  <h4 className={`text-base leading-snug ${!con.isRead ? 'font-black text-foreground' : 'font-bold text-muted-foreground'}`}>{con.title}</h4>
                  <div className="flex items-center gap-4 text-xs text-muted-foreground">
                    <span className="flex items-center gap-1.5"><Calendar className="h-3.5 w-3.5" /> Nhận ngày: {con.assignedDate}</span>
                    <span className="flex items-center gap-1.5 font-bold text-amber-600"><Clock className="h-3.5 w-3.5" /> Hạn góp ý: {con.deadline}</span>
                  </div>
                </div>

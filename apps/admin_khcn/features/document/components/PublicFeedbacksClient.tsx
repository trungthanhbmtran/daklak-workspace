    ...
    <div className="p-6 space-y-6 bg-muted/5 min-h-screen">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">Kiểm duyệt và phê duyệt ý kiến đóng góp từ người dân, doanh nghiệp.</p>
        </div>
      </div>
        ...
        <CardContent className="p-0">
          <div className="divide-y divide-border/50">
            {isLoading ? (
              ...
              <div key={fb.id} className={`p-5 transition-colors ${fb.status === 'APPROVED' ? 'bg-emerald-50/20' : fb.status === 'REJECTED' ? 'bg-red-50/20' : 'bg-background hover:bg-muted/10'}`}>
                <div className="flex justify-between items-start mb-4">
                  ...
                  <Badge className={`shadow-none font-bold text-[10px] uppercase px-2 py-0.5 ${fb.status === 'APPROVED' ? 'bg-emerald-100 text-emerald-700' : fb.status === 'REJECTED' ? 'bg-red-100 text-red-700' : 'bg-amber-100 text-amber-700'}`}>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
                  ...
                  <div className="lg:col-span-3 space-y-4">
                    ...
                    <div className="flex items-center gap-2 pt-2">
                      {fb.status === "PENDING" && (
                        <>
                          <Button size="sm" className="bg-emerald-600 hover:bg-emerald-700 text-white shadow-sm font-bold text-xs h-9 px-4" onClick={() => handleModerate(fb.id, 'APPROVED')} disabled={isActionLoading}>
                            <CheckSquare className="h-4 w-4 mr-1.5" /> Phê duyệt cho hiển thị
                          </Button>
                          <Button variant="outline" size="sm" className="text-red-600 border-red-100 hover:bg-red-50 font-bold text-xs h-9 px-4" onClick={() => handleModerate(fb.id, 'REJECTED')} disabled={isActionLoading}>

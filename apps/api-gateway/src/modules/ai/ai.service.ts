...
export class AiService implements OnModuleInit {
  ...
  private async getProviders(): Promise<AiProviderConfig[]> {
    try {
      const response = await firstValueFrom(this.configService.GetConfigs({})) as any;
      const configs = response.configs || [];
      const aiProvidersConfig = configs.find((c: any) => c.key === 'AI_PROVIDERS');
  ...
  async generateText(prompt: string): Promise<string> {
    ...
    if (providers.length === 0) {
      throw new Error('Không có cấu hình AI nào khả dụng hoặc đang được bật trong hệ thống.');
    }
    ...
    for (const provider of providers) {
      try {
        this.logger.log(`Attempting to use AI Provider: ${provider.provider} (Priority: ${provider.priority}, Model: ${provider.model})`);
        
        const result = await this.callProvider(provider, prompt);
        
        this.logger.log(`Successfully generated response using ${provider.provider}`);
        return result;
      } catch (err: any) {
        this.logger.warn(`Failed using ${provider.provider} (Priority: ${provider.priority}). Error: ${err.message}`);
        lastError = err;
    ...
    throw new Error('Tất cả các dịch vụ AI đều đang bận hoặc lỗi. Chi tiết lỗi cuối cùng: ' + lastError?.message);
  }

  private async callProvider(config: AiProviderConfig, prompt: string): Promise<string> {

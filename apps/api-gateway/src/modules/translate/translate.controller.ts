...
import { type ClientGrpc, ClientProxy, EventPattern } from '@nestjs/microservices';
...
export class TranslateController implements OnModuleInit {
  ...
  async translate(@Body() body: { text: string; targetLang: string }) {
    if (!body.text || !body.targetLang) {
      return { status: 'error', message: 'Missing text or targetLang' };
    }
    ...
    try {
      ...
      await this.redisService.set(`translate_job_${jobId}`, JSON.stringify({ status: 'PROCESSING' }), 3600);
      
      // Emit event to RabbitMQ
      this.rmqClient.emit('translate_task', { jobId, text: body.text, targetLang: body.targetLang });
      
      // Return 202 immediately
      return { status: 'success', data: { jobId, status: 'PROCESSING' } };
    } catch (err: any) {
      this.logger.error('Error queuing translation task', err);
      return { status: 'error', message: 'Không thể tạo tác vụ dịch thuật' };
    }
  }
  ...
  async handleTranslateTask(data: { jobId: string; text: string; targetLang: string }) {
    ...
    try {
      ...
      await this.redisService.set(`translate_job_${data.jobId}`, JSON.stringify({ 
        status: 'COMPLETED', 
        result 
      }), 3600);

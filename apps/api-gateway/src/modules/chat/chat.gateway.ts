...
export class ChatGateway implements OnGatewayConnection, OnGatewayDisconnect {
  ...
  async handleConnection(client: Socket) {
    try {
      ...
      if (isNaN(userId)) {
        this.logger.warn(
          `Client disconnected: Invalid token payload (${client.id})`,
        );
        client.disconnect();
        return;
      }
      ...
      const employeeCode =
        decoded.employeeCode || decoded.employee_code || String(userId);
  ...
  async handleDisconnect(client: Socket) {
    ...
    if (user && user.employeeCode) {
      this.logger.log(
        `Client disconnected: ${user.employeeCode} (${client.id})`,
      );
  ...
  handleTyping(
    ...
    const user = (client as any).user;
    if (user) {
      client.to(`room_${data.conversationId}`).emit('typing', {
  ...
  handleStopTyping(
    ...
    const user = (client as any).user;
    if (user) {
      client.to(`room_${data.conversationId}`).emit('stop_typing', {
        conversationId: data.conversationId,
        userId: user.employeeCode,
      });
    }
  ...
  broadcastMessage(conversationId: string, message: any) {
    ...
    if (this.server) {
      ...
      this.logger.warn(
        `WebSocket server is not initialized yet. Skipping broadcast.`,
      );
    }
  }
}

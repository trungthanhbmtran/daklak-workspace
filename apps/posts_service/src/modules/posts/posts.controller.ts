...
export class PostsController {
  ...
  async addTagsToPost(data: { postId: string; tagIds: string[] }) {
    const result = await this.postsService.addTagsToPost(
      data.postId,
      data.tagIds,
    );
  ...
  async removeTagFromPost(data: { postId: string; tagId: string }) {
    const result = await this.postsService.removeTagFromPost(
      data.postId,
      data.tagId,
    );
    return { success: true, data: result };
  ...
  async setCategoryForPost(data: { postId: string; categoryId: string }) {
    const result = await this.postsService.setCategoryForPost(
      data.postId,
      data.categoryId,
    );
    return { success: true, data: result };
